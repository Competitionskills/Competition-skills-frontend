...
const normalizeId = (v: any) =>
  String(
    v?._id ?? v?.id ?? (typeof v === "object" ? v?.toString?.() : v) ?? ""
  );
...
const OverviewCompactCard: React.FC<{
  ...
  const progress = Math.max(
    0,
    Math.min(1, (totalPoints % pointsPerTicket) / pointsPerTicket)
  );
  const remainder = totalPoints % pointsPerTicket;
  const ptsToNext =
    remainder === 0 ? pointsPerTicket : pointsPerTicket - remainder;
...
const MainContent: React.FC<MainContentProps> = ({
  ...
  const displayName = useMemo(
    () => displayNameFromUser(user, userName),
    [user, userName]
  );
  ...
  const handleClaimDaily = async (
    ...
    if (ticketsAwarded)
      updatePrestigeTickets(userPrestigeTickets + ticketsAwarded);
    updateLoginStatus({
      ...loginStatus,
    ...
    try {
      ...
      if (!res.ok)
        console.warn("Daily claim API returned non-OK; kept optimistic UI.");
      await fetchAll();
    } catch (e) {
      ...
      <main className="w-full py-6 px-4 md:px-6 pb-20 md:pb-10">
        ...
        <section className="mb-6">
          <div className="rounded-2xl border border-indigo-100 bg-white/70 backdrop-blur-sm p-5 shadow-sm">
            ...
            <p className="text-sm md:text-base text-slate-600 mt-1">
              {typeof loginStatus?.currentStreak === "number" &&
              loginStatus.currentStreak > 0
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          <OverviewCompactCard
            ...
            prestigeTickets={
              (user as any)?.prestigeTickets ?? userPrestigeTickets
            }
            referrals={
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <div
            id="my-competitions"
            className="bg-white shadow-lg rounded-xl p-4 md:p-6 border border-indigo-100"
          >
